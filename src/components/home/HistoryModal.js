...
export default function HistoryModal({ visible, onClose }) {
  ...
  const loadHistory = async (nextPage = 1, reset = false) => {
    ...
    try {
      ...
      setError(err?.message || 'Gagal mengambil riwayat perjalanan.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatVehicle = (item) => {
    if (!item.vehicle_type) return null;
    const typeStr = item.vehicle_type === 'motorcycle' ? 'Motor' : 'Mobil';
    const countStr = item.vehicle_count ? ` · ${item.vehicle_count} Unit` : '';
